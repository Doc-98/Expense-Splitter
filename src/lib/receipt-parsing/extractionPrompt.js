...
//
// `sourceKind` — 'image' (default), 'document' or 'text', from
// mediaKind.js's mediaKindFor() — only changes the opening sentence, since
// a receipt scanned as a PDF or a text/HTML export needs a different
// framing than "a photo" but the actual extraction rules below apply
// identically either way.
const INTROS = {
  image: 'You are reading a photo of a shopping/grocery receipt.',
  document: 'You are reading a shopping/grocery receipt from an uploaded document.',
  text: 'Below is the text of a shopping/grocery receipt — it may include HTML markup; ignore any tags and read only the visible text.',
}

// `categoryNames` is the group's own category list — when the model's
// already looking at the receipt to extract items, asking it to also guess
// each item's category is nearly free (same call, same attachment), and
...
export function buildExtractionPrompt(categoryNames = [], sourceKind = 'image') {
...
Receipts abbreviate and generalize constantly — a line might read "FR ROSSA" for red fruit, or lump a whole counter under one word like "DELI" or the Italian "GASTRONOMIA" rather than naming the actual product. That's fine: you don't need to know exactly what the item is, just its most likely broad category. Use null whenever you're not reasonably confident — every item gets reviewed by a person after scanning anyway, so a confidently wrong guess is worse than an honest blank.`
  ...
  return `${INTROS[sourceKind] || INTROS.image} Extract every purchased line item.
